...
instance (Position a) => Position (Qualifier a) where
    pos (Qual a _   ) = pos a
    pos (Quals a _ _) = pos a
    pos (ThisQual a ) = pos a
    pos (DataQual a ) = pos a
...
instance (Position a) => Position (Expression a) where
    ...
    pos (CtorExpr a _   ) = pos a
    pos (QObjExpr  a _ _) = pos a
...
instance (Position a) => Position (Statement a) where
    ...
    pos (QAssStmt  a _ _ _    ) = pos a
    pos (QAddStmt  a _ _ _    ) = pos a
    pos (QSubStmt  a _ _ _    ) = pos a
    pos (QMulStmt  a _ _ _    ) = pos a
    pos (QDivStmt  a _ _ _    ) = pos a
    pos (QPowStmt  a _ _ _    ) = pos a
    pos (QCompStmt a _ _ _    ) = pos a
