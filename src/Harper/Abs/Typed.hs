...
instance (Typed a) => Typed (Qualifier a) where
    typ (Qual a _   ) = typ a
    typ (Quals a _ _) = typ a
    typ (ThisQual a ) = typ a
    typ (DataQual a ) = typ a
...
instance (Typed a) => Typed (Expression a) where
    ...
    typ (CtorExpr a _   ) = typ a
    typ (QObjExpr  a _ _) = typ a
...
instance (Typed a) => Typed (Statement a) where
    ...
    typ (QAssStmt  a _ _ _    ) = typ a
    typ (QAddStmt  a _ _ _    ) = typ a
    typ (QSubStmt  a _ _ _    ) = typ a
    typ (QMulStmt  a _ _ _    ) = typ a
    typ (QDivStmt  a _ _ _    ) = typ a
    typ (QPowStmt  a _ _ _    ) = typ a
    typ (QCompStmt a _ _ _    ) = typ a
