...
issue2_lamPar = TProg "\
\f n = return (\\val x val y => n + x + y);\n\
\\n\
\appMul a b = (\\val n => f n (a * b));\n\
\\n\
...
\main = 42 / 0;\
\"
    (Out "\ESC[38;2;255;0;0mError: value `0` evaluated to zero causing a division error.\n\
...
\f n = 42 mod n;\
\"
    (Out "\ESC[38;2;255;0;0mError: value `n` evaluated to zero causing a division error.\n\
    \During evaluation of:\n\
    \ 42 mod n\n\
...
\f x y = x * y;\n\
\"
    (Out "\ESC[38;2;255;0;0mError: value `y` has invalid type 'String'; expected 'Integer'.\n\
    \During evaluation of:\n\
    \ x * y\n\
...
\main = (f 17) + (f 18);\n\
\"
    (Out "\ESC[38;2;255;0;0mError: control reached end of function without a return statement.\n\
    \During evaluation of:\n\
    \ {\n\
    \  if n <= 17 {\n\
    \    return 42 ;\n\
...
\f n = (\\val x val y => n + x + y);\n\
\\n\
\appMul a b = (\\val n => f n (a * b));\n\
\\n\
\main = appMul 7 17 42 5 1;\n\
\"
    (Out "\ESC[38;2;255;0;0mError: function applied to too many arguments.\n\
    \During evaluation of:\n\
    \ 1\n\
...
\main = f 42;\n\
\"
    (Out "\ESC[38;2;255;0;0mError: value `1` has invalid type 'Integer'; expected 'Bool'.\n\
    \During evaluation of:\n\
    \ if 1 {\n\
    \   return n ;\
    \ }\n\
...
\main = f 17 ();\n\
\"
    (Out "\ESC[38;2;255;0;0mError: values `a` and `b` have invalid types 'Integer', 'Unit'; \
...
testResult (TProg i (Out _ exp)) = TestCase (assertEqual "Expected result" exp res)
    where Out _ res = let ts = lexer i
                      in  case parser ts of
                            Bad e -> error e
                            Ok  t -> runInterpreter t

testAll :: TestProgram -> Test.HUnit.Test
testAll (TProg i exp) = TestCase (assertEqual "Expected output and result" (san exp) (san res))
    where Out o r = let ts = lexer i
                    in  case parser ts of
                          Bad e -> error e
                          Ok  t -> runInterpreter t
          res = Out (o "") r
          san (Out o r) = Out (fst $ foldr f ("", False) o) r
              where f c (acc, ws) = case (isSpace c, ws) of
                                        (True, True)  -> (acc, True)
                                        (True, False) -> (' ':acc, True)
                                        (False, _)    -> (c:acc, False)

tests = TestList [TestLabel "issue1_sqr" (testResult issue1_sqr),
