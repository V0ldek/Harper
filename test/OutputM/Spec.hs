...
testFailure = FTCase (test 42) exp
    where test :: Integer -> Output String Integer
          test n = if n == 37 then fail "Reached 37" else do
...
testProg (FTCase o exp) = TestCase (assertEqual "Expected output and result" exp o)
testProg (OTCase (Out o _) exp) = TestCase (assertBool "Expected output" (exp `isPrefixOf` o))

tests = TestList [TestLabel "testSuccess" (testProg testSuccess),
